(() => {
  ...
  function hallDescriptionHtml() {
    const f = state.flags;
    const beam = f.flashlightOn && f.flashlightHasFreshBatteries && hasItem("Emergency flashlight")
      ? "Thomas's flashlight throws a clean beam along the landing."
      : "Most of the landing remains in shadow, with only weak daylight reaching it from the stairwell.";
    return `
      <p>The upstairs landing outside the master bedroom is narrow and markedly darker than the bedroom. ${beam}</p>
      <p>A ceiling light fixture hangs overhead. A wall switch sits beside the bedroom doorway. Neither shows obvious damage.</p>
      <p>Other doors recede along the hall, but the darkness makes the far end difficult to read without a working light.</p>
    `;
  }
  ...
  function renderHall() {
    const f = state.flags;
    const beam = f.flashlightOn && f.flashlightHasFreshBatteries && hasItem("Emergency flashlight")
      ? "Thomas's repaired flashlight is on, its beam reaching cleanly down the landing."
      : "Very little daylight reaches the landing from the stairwell, leaving the far end in shadow.";
    sceneEl.innerHTML = `
      <p>Thomas steps into the upstairs hall. ${beam}</p>
      <p>A ceiling fixture hangs above the landing. The light switch beside the master bedroom door is in easy reach. The fixture remains dark.</p>
      <p>Several doors continue along the hall. For now, the practical problem is simpler: the upstairs light is not working.</p>
    `;
  }
  ...
  function handleBedroomAuditObjects(q, raw) {
    ...
    if ((isLook || hasAny(q, ["read floor plan", "read house plan", "open floor plan", "unfold floor plan", "view floor plan"]) || q === "floor plan") &&
      ...
      if (!f.floorPlanTaken) {
        ...
        say("Thomas unfolds the architectural plan. It shows the ground floor, upper floor, basement and attic with room dimensions. At first glance it looks ordinary.", raw);
        openFloorPlanPanel();
      }
      return true;
    }
  ...
  function bedroomCommand(q, raw) {
    ...
    const photoAvailable = f.boxUnlocked || f.photoTaken || hasItem("Trimmed photograph of Thomas and Jennifer");
    const photoClueCommand =
      q === "photo" || q === "photograph" ||
      hasAny(q, ["look at photograph", "examine photograph", "inspect photograph", "check photograph", "study photograph", "look at photo", "examine photo", "inspect photo", "check photo", "study photo"]) ||
      hasAny(q, ["look at edge", "look at cut edge", "examine edge", "examine cut edge", "inspect edge", "inspect cut edge", "check edge", "study edge"]) ||
      hasAny(q, ["why is photo cut", "why is photograph cut", "why photo cut", "why photograph cut"]) ||
      (q === "look closer" && state.focus === "photo");

    if (photoClueCommand) {
      if (!photoAvailable) {
        say("Thomas has not found a loose photograph to examine yet.", raw);
      } else {
        state.focus = "photo";
        f.photoExamined = true;
        say("Thomas looks more carefully at the photograph. The cut along the right edge is uneven. Jennifer's right arm continues toward the missing side, positioned as though it had been around someone standing beside her. Someone has been physically cut out of the picture. Thomas cannot remember who, or why.", raw);
      }
      return;
    }

    const emptyFrameLook =
      q === "empty frame" ||
      hasAny(q, ["look at empty frame", "examine empty frame", "inspect empty frame", "check empty frame", "look at frame"]) && !hasAny(q, ["photo", "photograph"]);
    if (emptyFrameLook) {
      state.focus = "emptyFrame";
      f.emptyFrameExamined = true;
      say("The smaller frame hangs slightly crooked. The glass is intact, but there is no photograph inside it. The cardboard backing is still held in place by the little metal tabs. Nothing on the front explains why the frame is empty.", raw);
      return;
    }

    const emptyFrameBack =
      hasAny(q, ["look at back of frame", "look at back of empty frame", "check back of frame", "examine back of frame", "look at backing", "check backing", "examine backing", "open empty frame"]) ||
      (hasAny(q, ["look at back", "check back", "examine back"]) && state.focus === "emptyFrame");
    if (emptyFrameBack) {
      state.focus = "emptyFrame";
      f.emptyFrameExamined = true;
      f.emptyFrameBackChecked = true;
      f.frameOpened = q.includes("open");
    ...
    if (f.mattressFlipped && hasAny(q, ["look at bed frame", "examine bed frame", "inspect bed frame", "check bed frame", "look at slats", "look at bed slats", "examine slats", "examine bed slats", "inspect slats", "check slats", "look at support slats", "look at slat", "examine slat"])) {
      state.focus = "mattressSlat";
      f.mattressScratchSeen = true;
      if (f.mattressScratchCleaned) {
        say("With the mattress out of the way, the wooden support slats are exposed. On one slat, two very faint letters show through the cleaned patch: EM. They were scratched by hand, shallow and uneven.", raw);
      } else {
        say("With the mattress out of the way, the wooden support slats are exposed. One support bears a few shallow, deliberate scratches beneath a film of dust. They are too faint to read without looking more closely or clearing the dust.", raw);
      }
      return;
    }

    if (f.mattressFlipped && (
      hasAny(q, ["clean dust", "clean the dust", "wipe dust", "wipe the dust", "clean slat", "wipe slat", "clean dust off slat", "brush dust off slat"]) ||
      hasAny(q, ["examine scratching", "inspect scratching", "look at scratching", "examine scratches on slat", "inspect scratches on slat", "look closer at scratching"])
    )) {
      state.focus = "mattressSlat";
      f.mattressScratchSeen = true;
      f.mattressScratchCleaned = true;
      say("Thomas brushes the dust aside and leans closer. Two very faint letters have been scratched into the wooden support: EM. The marks are shallow and uneven, clearly made by hand. Initials, half a word, some old private notation. It could mean anything.", raw);
      return;
    }

    if (hasAny(q, ["check outlet", "look at outlet", "examine outlet", "inspect outlet", "check bedroom outlet", "look at bedroom outlet"])) {
      if (!f.tableMoved) {
        say("The bedroom outlet is mostly hidden behind the bedside table. Thomas can see the alarm clock cord running toward it, but he would need to move the table to inspect the socket properly.", raw);
      } else {
        state.focus = "bedroomOutlet";
        say("The outlet looks ordinary. No scorch marks, cracked plastic or loose faceplate. The alarm clock is connected to it and still dead. Whatever is wrong is not visibly wrong at the socket.", raw);
      }
      return;
    }

    if (hasAny(q, ["plug phone charger into outlet", "plug charger into outlet", "plug phone charger in", "plug charger in", "charge phone", "plug phone in", "plug phone into outlet"])) {
      if (!f.chargerTaken && !hasItem("Phone charger")) {
        if (f.tableMoved) {
          f.chargerTaken = true;
          addInventory("Phone charger");
        } else {
          say("Thomas needs the phone charger first. He remembers bringing it upstairs, but it is not visible from here.", raw);
          return;
        }
      }
      f.bedroomOutletTested = true;
    ...
    if (hasAny(q, ["turn on hall light", "turn hall light on", "switch on hall light", "flip hall light switch", "use hall light switch"]) && f.hallSeen) {
      say("Thomas reaches for the hall switch and flips it. Nothing. The ceiling fixture remains dark.", raw);
      return;
    }

    if (hasAny(q, ["check light switch", "examine light switch", "inspect light switch", "check hall switch", "look at light switch"]) && f.hallSeen) {
      f.hallLightSwitchChecked = true;
      say("The hall switch looks ordinary. The toggle moves normally and there is no obvious damage around the plate. It simply does not turn the ceiling light on.", raw);
      return;
    }

    if (hasAny(q, ["check bulb", "look at bulb", "examine bulb", "inspect bulb", "check hall bulb"]) && f.hallSeen) {
      f.hallBulbChecked = true;
      say("The bulb in the hall fixture is intact. The glass is unbroken and the filament does not appear snapped. The problem is probably not the bulb.", raw);
      return;
    }
    ...
    const floorPlanBedroomCompare = floorPlanNoun && hasAny(q, [
      "compare floor plan to bedroom", "compare floor plan with bedroom", "compare bedroom to floor plan",
      "compare plan to bedroom", "compare plan with bedroom", "compare bedroom to plan",
      "compare floor plan to room", "compare plan to room"
    ]);

    if (floorPlanBedroomCompare) {
      ...
      f.floorPlanBedroomCompared = true;
      say("The plan lists the master bedroom as 14 × 16 feet. Thomas paces the shorter wall and checks it again against the floorboards. Roughly, the room seems perhaps six inches wider than the drawing says. Old plans, plaster, trim, remodeling, or Thomas's terrible method of measuring could explain that. He does not give it much weight.", raw);
      return;
    }
    ...
    if (floorPlanTake || floorPlanInspect) {
      ...
      if (floorPlanTake) {
        say("Thomas unfolds it enough to confirm what it is. Ground floor. Upper floor. Basement. Attic. Room measurements. Nothing immediately unusual. He folds it again and keeps it.", raw);
      } else {
        say("Thomas unfolds the architectural plan. It shows the ground floor, upper floor, basement and attic with room dimensions. At first glance it looks ordinary.", raw);
      }
      openFloorPlanPanel();
    ...
    if (hasAny(q, ["open bedroom door", "open door"]) && !q.includes("bathroom")) {
      f.bedroomDoorOpen = true;
      f.hallSeen = true;
      if (f.flashlightHasFreshBatteries && hasItem("Emergency flashlight")) {
        say("Thomas opens the bedroom door. The upstairs hallway beyond it is considerably darker than the bedroom, and the ceiling light still does not respond. With a working flashlight available, the darkness is at least a practical problem he can solve.", raw);
      } else {
    ...
    if (hasAny(q, ["leave bedroom", "go hall", "go hallway", "enter hallway", "step into hall", "step into hallway"])) {
      f.bedroomDoorOpen = true;
      f.hallSeen = true;
    ...
    if (hasAny(q, ["look at empty frame", "examine empty frame", "inspect empty frame", "check empty frame"])) {
      state.focus = "emptyFrame";
      f.emptyFrameExamined = true;
      say("The smaller frame hangs slightly crooked. The glass is intact, but there is no photograph inside it. The cardboard backing is still held in place by the little metal tabs. Nothing on the front explains why the frame is empty.", raw);
      return;
  ...
  function extendedBedroomCommand(q, raw) {
    ...
    if (hasAny(q, ["look under flipped mattress", "look at bed slats", "examine bed slats", "look at support slats", "look at slat", "examine slat", "inspect slat", "check slat", "look at support slat"])) {
      if (!f.mattressFlipped) {
        ...
        say("The dust has been brushed away from one wooden support slat. Two very faint letters show in the cleaned patch: EM. They were scratched by hand, shallow and uneven. Thomas cannot tell what they were meant to be.", raw);
      } else {
        f.mattressScratchSeen = true;
    ...
    if (hasAny(q, ["clean slat", "clean slate", "wipe slat", "clean dust off slat", "wipe dust off slat", "brush dust off slat"]) ||
      ...
      if (!f.mattressFlipped) {
        ...
        f.mattressScratchSeen = true;
        f.mattressScratchCleaned = true;
        say("Thomas brushes the dust away with his fingertips. Two very faint letters emerge in the wood: EM. The marks are shallow and uneven, clearly made by hand. Initials, half a word, some old private notation. It could mean anything.", raw);
      }
      return true;
    }
  ...
  function hallCommand(q, raw) {
    ...
    if (hasAny(q, ["look around", "look at hall", "look at hallway", "examine hall", "examine hallway", "check hallway", "describe hall"])) {
      say("The upstairs landing is narrow and dark. A ceiling fixture hangs overhead, with a wall switch beside the master bedroom door. Several other doors continue along the hall, but weak daylight from the stairwell does little for the far end.", raw);
      return;
    }

    if (hasAny(q, ["turn on hall light", "turn hall light on", "turn on light", "switch on light", "flip light switch", "use light switch", "try light switch"])) {
      say("Thomas flips the hall switch. Nothing. The ceiling fixture remains dark.", raw);
      return;
    }

    if (hasAny(q, ["check light switch", "look at light switch", "examine light switch", "inspect light switch", "check switch", "examine switch"])) {
      f.hallLightSwitchChecked = true;
      say("The switch looks ordinary. The toggle moves normally, the plate is secure, and there are no scorch marks or cracks. It simply does not turn the hall light on.", raw);
      return;
    }

    if (hasAny(q, ["check bulb", "look at bulb", "examine bulb", "inspect bulb", "check light bulb", "look at light bulb"])) {
      f.hallBulbChecked = true;
      say("The bulb looks intact. The glass is unbroken and the filament does not appear snapped. The problem is probably not the bulb.", raw);
      return;
    }

    if (hasAny(q, ["turn on flashlight", "switch on flashlight", "use flashlight", "flashlight on"])) {
      if (!f.flashlightTaken && !hasItem("Emergency flashlight")) {
        say("Thomas does not have the flashlight with him.", raw);
      } else if (!f.flashlightHasFreshBatteries) {
        say("Thomas presses the flashlight switch. Nothing. It still needs working batteries.", raw);
      } else {
        f.flashlightOn = true;
        say("Thomas switches on the flashlight. A clean white beam reaches down the hallway.", raw);
      }
      return;
    }

    if (hasAny(q, ["turn off flashlight", "switch off flashlight", "flashlight off"])) {
      if (f.flashlightOn) {
        f.flashlightOn = false;
        say("Thomas switches the flashlight off. The far end of the hall disappears back into shadow.", raw);
      } else {
        say("The flashlight is already off.", raw);
      }
      return;
    ...
    if (hasAny(q, ["go bedroom", "go to bedroom", "enter bedroom", "return bedroom", "back to bedroom", "leave hallway", "leave hall"])) {
      f.bedroomDoorOpen = true;
      setRoom("bedroom");
      return;
    }

    if (hasAny(q, ["go farther", "go down hall", "walk down hall", "continue down hall", "explore hallway"])) {
      if (!f.flashlightOn) {
        say("Thomas can make out the landing, but not enough of the far end to wander around it sensibly. A working light would help.", raw);
      } else {
        say("The flashlight makes the rest of the landing readable, but Thomas stops at the cluster of closed doors. He has not decided which room to deal with next.", raw);
      }
      return;
    }
