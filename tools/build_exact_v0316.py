...
source = source.replace('const BUILD_VERSION = "v0.3.15";', 'const BUILD_VERSION = "v0.3.16";', 1)
...
new = '''    if (hasAny(q, ["leave bedroom", "go hall", "go hallway", "enter hallway", "step into hall", "step into hallway"])) {
      ...
      if (doorSlams) {
        ...
        say("The open bedroom window catches the change in pressure as Thomas steps into the hall. Behind him, the bedroom door slams shut hard enough to make him flinch. Cold air moving through the room is the obvious explanation. He stands still for another second anyway.", raw);
      }
      return;
    }
'''
...
<head>
  ...
  <link rel="stylesheet" href="styles.css?v=0.3.16" />
</head>
<body>
  <main class="app-shell">
    <section class="game" aria-label="What the House Kept">
      <header class="topbar">
        ...
        <div class="chapter">Chapter One · v0.3.16</div>
      </header>
  ...
  <script src="sfx-v0316.js?v=0.3.16"></script>
  <script src="game.js?v=0.3.16"></script>
</body>
</html>
''', encoding="utf-8")

(assets / "sfx-v0316.js").write_text(r'''(() => {
  "use strict";
...
(OUT / "README-v0.3.16.txt").write_text('''What the House Kept v0.3.16
Base: exact v0.3.15 INTERACTIONS runtime.
Opening music: absent. No music controller and no opening-music MP3 are bundled.
...
# Static regression checks for the user's actual v0.3.15 interaction pass.
required = [
...
print('Exact v0.3.16 Android project generated from v0.3.15 INTERACTIONS')
